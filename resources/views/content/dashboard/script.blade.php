...
==========================================================================================*/
    ...
    $(window).on('load', function () {
        ...
        earningsChartOptions = {
            ...
            series: [@foreach($month_item_quantity as $iq){{$iq->quantity_sum}}, @endforeach],
            legend: { show: true },
            comparedResult: [2, -3, 8],
            labels: [@foreach($month_item_quantity as $iq)'{{$iq->name}}', @endforeach],
            stroke: { width: 0 },
            colors: [@foreach($month_item_quantity as $iq)generateDarkColorHex(),@endforeach],
            grid: {
                padding: {
                    right: -20,
                    bottom: -8,
        ...
        revenueReportChartOptions = {
            ...
            series: [
                {
                    name: 'Earning',
                    data: [@foreach($sales_trend as $st)'{{$st->sum_total}}', @endforeach]
                }
            ],
            dataLabels: {
                enabled: true
            ...
            xaxis: {
                categories: [@foreach($sales_trend as $st)'{{$st->month_name}}', @endforeach],
                labels: {
                    style: {
